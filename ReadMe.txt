...
Если работает DHCP
	...
	5) если есть rescue Linux ==> тогда в vKVM грузим ISO MfsBSD, пробрасываем /dev/sda, через ssh или VNC клиент устанавливаем с ISO систему, потом правим сеть и пробуем перегрузить хост машину.


Если НЕ работает DHCP

	1) если есть установленная Linux ==> то через GRUB, ISO, kFreeBSD со сменой сетевых настроек
	2) есть rescue FreeBSD с ZFS ==> перепаковываем MfsBSD.img и потом пишем на /dev/ada0
	3) если есть возможность грузить ISO ==> модефицируем MfsBSD, грузимся с нашего образа и с него ставим через go11_4k.sh
